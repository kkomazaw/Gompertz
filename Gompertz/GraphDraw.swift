...
class GraphDraw: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        valueArray.append(actualA)
        valueArray.append(actualB)
        valueArray.append(actualC)
        if hasTx, !vFtx.isInfinite {
            valueArray.append(vFtx)
        }
        ...
        var vF0 = exp(lnGmax + (a - lnGmax) * exp(-k * (-t0)))
        if tx < 0 {
            vF0 = exp(lnGmax + (a - lnGmax) * exp(-k * (tx - t0)))
        }
        if vF0.isInfinite {
            vF0 = exp(a)
        }
